...
export interface backCloseModalAction {
...
export type ModalActions =
  ...
  | deleteModalAction
  | clearModalAction
  | backCloseModalAction;

export const createModal = (
  props: Omit<ModalType, 'component'>,
): createModalAction => ({
  type: ActionType.create,
  payload: props,
});
...
export const backCloseModal = (): backCloseModalAction => ({
  type: ActionType.back,
});
...
const deleteModalState = (state: ModalType[], action: any) => {
  ...
  if (closeTimeoutMS > 0) {
    ...
    return state.filter(
      (modal: ModalType) => modal.name !== action.payload.name,
    );
  }
...
const backCloseModalState = (state: ModalType[]) => {
  let modals: any[] = state;
  let findModals = modals.filter((modal: ModalType) => !modal.props.isBack);
  let name = findModals[findModals.length - 1].name;

  if (name) {
    return state.filter((modal: ModalType) => modal.name !== name);
  }
...
export const reducer = (state: ModalType[], action: any): ModalType[] => {
  ...
  switch (type) {
    ...
    case ActionType.back:
      return backCloseModalState(state);
