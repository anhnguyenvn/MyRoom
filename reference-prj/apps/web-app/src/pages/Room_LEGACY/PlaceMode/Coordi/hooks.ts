...
const useCoordi = () => {
  ...
  const handleClickAddCoordi = useCallback(() => {
    console.log('handleClickAddCoordi ')
    if (!meRoomId) {
      return;
    }
  ...
  const handleClickChangeCoordi = useCallback(async () => {
    console.log('handleClickChangeCoordi ', meRoomId && selectedId)
  ...
  const handleClickCoordi = useCallback( (id: string) => async () => {
    console.log('Handle Clk ', id)
    console.log('Handle Clk ', id)
    setSelectedId(id);
    if (meRoomId) {
      const manifest = await mutationFetchMyroomTemplate.mutateAsync({
